...
std::vector<struct Token> LexerNew::lex(const std::string& code, const std::vector<struct ReservedWordNew>& reserved_words) {
    m_code = code;
    m_line = 1;
    m_current = 0;
...
void LexerNew::skip_ws() {
    while (m_code[m_current] == ' ')
        m_current++;
}

bool LexerNew::is_digit(char c) {
    return '0' <= c && c <= '9';
}

bool LexerNew::is_char(char c) {
    return ('A' <= c && c <= 'Z') || ('a' <= c && c <= 'z');
}

struct Token LexerNew::read_word() {
...
struct Token LexerNew::read_number() {
    struct Token t;
    t.start = &m_code[m_current];
    t.len = 1;
    t.line = m_line;
...
struct Token LexerNew::next_token() {
    skip_ws();
